...
#bug notes for v8 start:
#sub3 parentID values are incorrect, they are one off.

#DISCOVERY: it seems that the parentID scores are offset by 1 in relation to my data,
...
for element in paragraphs_strings:
    ...
    if(i >= 6 and i < 600):
        ...
        try:
            ...
            print("paragraphs_strings[i] = ", paragraphs_strings[i])
            print("paragraphs[i] = ", paragraphs[i])
            i = i + 1
            continue
        ...
        try:
            ...
            print("no prevsub, exception type 4. at i =", i)
            input("...")
            parentID.append(last_parent)
            i = i + 1
            continue #yes, continuing here is proper
            
            
            
        if(mysub_int < 0 or mysub_int > 3):
            print("mysub_int for i =", i, " was not a proper 0/1/2/3 value!")
            print("mysub_int =", mysub_int)
        ...
        if(mysub_int == 0):
            ...
            #sub0 nodes will have parent 6 (its parent is the base node!)
            last_sub0 = i 
            last_parent = 6
            if(i == 6):
                parentID.append(0)
        ...
        elif(mysub_int == 3 and prevsub_int == 2):
            #last_parent = last_parent
            parentID.append(last_parent)
            
        elif(mysub_int == 3):
            last_parent = last_sub2
            parentID.append(last_parent)
            
        else:
            # but if the two values are the same, the parent
            # will not change this round.
            input("Something strange happened! Error 6.")
            parentID.append(last_parent)
    else:
        ...
        if(i > 599):
            ...
            parentID.append(-2)
    print("last_parent was: ", last_parent, end = '')
    print(" for i = ", i ,"... and mysub_int = ", mysub_int)
    print("last_sub0 =",last_sub0)
    print("last_sub1 =",last_sub1)
    print("last_sub2 =",last_sub2)
    print()
    i = i + 1
...
for text in paragraphs:
    if(i > 5 and i < 601):
        try:
            ...
            dataset_values[datai+5], parentID[i]))
        except:
